...
import ProjectsPluginClient from './client-entry';
...
async function bootstrap() {
  console.log('[Projects Plugin] Starting in standalone mode...');

  try {
    // Create plugin instance (client-only version)
    const plugin = new ProjectsPluginClient();
    console.log('[Projects Plugin] Plugin instance created:', plugin);
    ...
    const runtime = await bootstrapStandalone(plugin, {
      ...
      apiBaseUrl: '/api',
      loggerPrefix: '[Projects Standalone]',
    });
    
    console.log('[Projects Plugin] Runtime bootstrapped:', runtime);
    ...
    console.log('[Projects Plugin] Standalone mode started successfully');
  } catch (error) {
    console.error('[Projects Plugin] Bootstrap failed:', error);
    // Show error on page
    const rootElement = document.getElementById('root');
    if (rootElement) {
